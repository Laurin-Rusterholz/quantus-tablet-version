(function () {
  ...
  async function handleSubmit(form) {
    ...
    if (type === "note") {
      ...
      /* Der Zugangsschluessel des geplanten Mailversands. Er liegt NUR hier im
         Geraet (lokale Einstellungen), steht in keiner Adresse und in keinem
         Quelltext. Ein leeres Feld laesst den bisherigen Wert stehen — nur
         „-" loescht ihn, damit ein versehentlich leeres Formular nicht
         stillschweigend den Zugang entfernt. */
      const tokenFeld = String(data.get("aiSyncToken") || "").trim();
      if (tokenFeld === "-") state.settings.aiSyncToken = "";
      else if (tokenFeld) state.settings.aiSyncToken = tokenFeld;
  ...
  window.__quantusTablet = {
    ...
    /* Kopfzeilen fuer die fail-closed gesicherten Endpunkte der Hauptapp.
       Ohne hinterlegten Schluessel bleibt es leer — dann antwortet der Server
       „gesperrt", und das ist die Absicht. */
    authHeaders: () => {
      const t = String((state.settings && state.settings.aiSyncToken) || "").trim();
